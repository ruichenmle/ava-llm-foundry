...
def get_hf_tokenizer_from_composer_state_dict(
    ...
    if hf_tokenizer_state != {}:
        tmp_output_dir = './saved_tokenizer/'
        os.makedirs(tmp_output_dir)
        for filename, saved_content in hf_tokenizer_state.items():
            tokenizer_file_path = Path(
                tmp_output_dir) / f'{filename}{saved_content["file_extension"]}'
        ...
        hf_tokenizer = AutoTokenizer.from_pretrained(tmp_output_dir)
...
def main(args: Namespace) -> None:
    ...
    config = AutoConfig.from_pretrained(local_folder_path)
    loaded_hf_model = AutoModelForCausalLM.from_pretrained(
        local_folder_path,
        config=config,
    )
